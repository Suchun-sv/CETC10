...
int num_road = 0;
fiber f[3];
//gg邻接矩阵存图，pre增广路径中每个点的前驱，flow源点到这个点的流量 
...
//	    	int xx = 0;
//    	xx = 1;
//    while (!q.empty()) q.pop();//q存该点
//    for (int i=1; i<=n; i++) pre[i]=-1;
//    pre[s]=0;
//    q.push(s);
//    if(xx) cout<<"new run is_for bfs:*************"<<endl;
//    if(xx) cout<<s<<endl;
//    flow[s]=INF;
//    while (!q.empty())
//    {
...
//        if(xx)  	cout<<i<<endl;
//            pre[i]=x;
//            flow[i]=min(flow[x],gg[x][i]);//存的是遍历到i点的最小流
//            q.push(i);//加入队列，继续往下遍历
//          }
//          else if(gg[x][i]<0 && g[x][i]>0)
//          {
//          	cout<<"flag"<<endl; 
//            up_the_road(x,i);
//          }
...
//void used[50][50]={0};
stack<int> ss;
int num[]={7,6,14,9,20,11,13,2,15};//所需的城市 
int maxx[500]={0};
void showPath();
int update_flag = 1;//标记最大流的更新是否开始 
int dfs_update(int i,int j)//这里的i是终点广州，j是源点50 
{
/*从后往前遍历，更新gg[*][*]为0的*/
	int used[500]={0};
	int que[50][50];
...
//		while(flag&&cnt--)
//		{
//			up_the_road(que[cnt][0],que[cnt][1]);
//		if(x)	cout<<"flag"<<" "<<que[cnt][0]<<" "<<que[cnt][1]<<endl;			
//		}
...
freopen("C:/Users/hsxny/Desktop/shuru.txt","r",stdin);
    ...
    maxflow = 0;
    int x = 0;
//if(update_flag == 1)   x = 1;
   
    int increase=0;
  if(x)  cout<<endl<<"new_time****************************"<<endl;
    while ((increase=dfs(s,t))!=-1)//这里的括号加错了！Tle 
    {//迭代 
...
freopen("C:/Users/hsxny/Desktop/shuru.txt","r",stdin);
    ...
    while(scanf("%d%d",&x,&y)==2&&x+y!=0)
    {
        ...
        path[i].k = 0;
        update(path[i]);
//        g[path[i].x][path[i].y] = 1000000;
//        cout<<path[i].x<<" "<<path[i].y<<" "<<g[path[i].x][path[i].y]<<endl; 
        i++;
    }
    n=1;
    num_road = i;
